...
export const EntryModal: React.FC<EntryModalProps> = ({ isOpen, onClose, onSave, onDelete, initialData, defaultDay }) => {
    ...
    return createPortal(
        <AnimatePresence>
            {isOpen && (
                <>
                    {/* Backdrop - Optimized: No blur on mobile to save GPU */}
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                    ...
                    {/* Sheet - Optimized: Simpler transition & hardware acceleration */}
                    <motion.div
                        initial={{ y: "100%" }}
                        animate={{ y: 0 }}
                        exit={{ y: "100%" }}
                        transition={{ type: "tween", ease: "circOut", duration: 0.3 }}
                        className="fixed bottom-0 left-0 right-0 bg-card z-[201] rounded-t-2xl p-6 shadow-xl max-w-md mx-auto max-h-[90vh] overflow-y-auto will-change-transform"
                    >
